'use strict';

Object.defineProperty(exports, '__esModule', { value: true });

var classPrivateFieldGet = require('../../../dist/classPrivateFieldGet-4a1dde25.cjs.prod.js');
var authClient = require('@dfinity/auth-client');
var agent = require('@dfinity/agent');

/* babel-plugin-inline-import '../assets/dfinity.svg' */
// @ts-ignore
const dfinityLogoLight = "data:image/svg+xml;base64,"; // @ts-ignore

/* babel-plugin-inline-import '../assets/dfinity.svg' */
const dfinityLogoDark = "data:image/svg+xml;base64,";

var _config = /*#__PURE__*/new WeakMap();

var _identity = /*#__PURE__*/new WeakMap();

var _principal = /*#__PURE__*/new WeakMap();

var _client = /*#__PURE__*/new WeakMap();

class InternetIdentity {
  get principal() {
    return classPrivateFieldGet._classPrivateFieldGet(this, _principal);
  }

  get client() {
    return classPrivateFieldGet._classPrivateFieldGet(this, _client);
  }

  constructor() {
    let userConfig = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

    classPrivateFieldGet._defineProperty(this, "meta", {
      features: [],
      icon: {
        light: dfinityLogoLight,
        dark: dfinityLogoDark
      },
      id: "ii",
      name: "Internet Identity"
    });

    classPrivateFieldGet._classPrivateFieldInitSpec(this, _config, {
      writable: true,
      value: void 0
    });

    classPrivateFieldGet._classPrivateFieldInitSpec(this, _identity, {
      writable: true,
      value: void 0
    });

    classPrivateFieldGet._classPrivateFieldInitSpec(this, _principal, {
      writable: true,
      value: void 0
    });

    classPrivateFieldGet._classPrivateFieldInitSpec(this, _client, {
      writable: true,
      value: void 0
    });

    classPrivateFieldGet._classPrivateFieldSet(this, _config, {
      whitelist: [],
      host: window.location.origin,
      providerUrl: "https://identity.ic0.app",
      dev: true,
      ...userConfig
    });
  }

  set config(config) {
    classPrivateFieldGet._classPrivateFieldSet(this, _config, { ...classPrivateFieldGet._classPrivateFieldGet(this, _config),
      ...config
    });
  }

  get config() {
    return classPrivateFieldGet._classPrivateFieldGet(this, _config);
  }

  async init() {
    classPrivateFieldGet._classPrivateFieldSet(this, _client, await authClient.AuthClient.create());

    const isConnected = await this.isConnected(); // // TODO: fix?

    if (isConnected) {
      var _classPrivateFieldGet2;

      classPrivateFieldGet._classPrivateFieldSet(this, _identity, classPrivateFieldGet._classPrivateFieldGet(this, _client).getIdentity());

      classPrivateFieldGet._classPrivateFieldSet(this, _principal, (_classPrivateFieldGet2 = classPrivateFieldGet._classPrivateFieldGet(this, _identity)) === null || _classPrivateFieldGet2 === void 0 ? void 0 : _classPrivateFieldGet2.getPrincipal().toString());
    }

    return true;
  }

  async isConnected() {
    return await classPrivateFieldGet._classPrivateFieldGet(this, _client).isAuthenticated();
  }

  async createActor(canisterId, idlFactory) {
    // TODO: pass identity?
    const agent$1 = new agent.HttpAgent({ ...classPrivateFieldGet._classPrivateFieldGet(this, _config),
      identity: classPrivateFieldGet._classPrivateFieldGet(this, _identity)
    });

    if (classPrivateFieldGet._classPrivateFieldGet(this, _config).dev) {
      // Fetch root key for certificate validation during development
      agent$1.fetchRootKey().catch(err => {
        console.warn("Unable to fetch root key. Check to ensure that your local replica is running");
        console.error(err);
      });
    } // TODO: add actorOptions?


    return agent.Actor.createActor(idlFactory, {
      agent: agent$1,
      canisterId
    });
  }

  async connect() {
    var _classPrivateFieldGet4;

    await new Promise((resolve, reject) => {
      var _classPrivateFieldGet3;

      (_classPrivateFieldGet3 = classPrivateFieldGet._classPrivateFieldGet(this, _client)) === null || _classPrivateFieldGet3 === void 0 ? void 0 : _classPrivateFieldGet3.login({
        // TODO: local
        identityProvider: classPrivateFieldGet._classPrivateFieldGet(this, _config).providerUrl,
        onSuccess: () => resolve(true),
        onError: reject
      });
    });
    const identity = (_classPrivateFieldGet4 = classPrivateFieldGet._classPrivateFieldGet(this, _client)) === null || _classPrivateFieldGet4 === void 0 ? void 0 : _classPrivateFieldGet4.getIdentity();
    const principal = identity === null || identity === void 0 ? void 0 : identity.getPrincipal().toString();

    classPrivateFieldGet._classPrivateFieldSet(this, _identity, identity);

    classPrivateFieldGet._classPrivateFieldSet(this, _principal, principal);

    return true;
  }

  async disconnect() {
    var _classPrivateFieldGet5;

    await ((_classPrivateFieldGet5 = classPrivateFieldGet._classPrivateFieldGet(this, _client)) === null || _classPrivateFieldGet5 === void 0 ? void 0 : _classPrivateFieldGet5.logout());
    return true;
  }

}

exports.InternetIdentity = InternetIdentity;
