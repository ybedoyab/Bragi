import { _ as _classPrivateFieldGet, a as _defineProperty, b as _classPrivateFieldInitSpec, c as _classPrivateFieldSet } from '../../../dist/classPrivateFieldGet-c8fedd89.esm.js';
import { IC } from '@astrox/connection';
import { PermissionsType } from '@astrox/connection/lib/esm/types';

/* babel-plugin-inline-import '../assets/astrox_light.svg' */
// @ts-ignore
const astroXLogoLight = "data:image/svg+xml;base64,"; // @ts-ignore

/* babel-plugin-inline-import '../assets/astrox.png' */
const astroXLogoDark = "data:image/png;base64,";

const balanceFromString = function (balance) {
  let decimal = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 8;
  const list = balance.split(".");
  const aboveZero = list[0];
  const aboveZeroBigInt = BigInt(aboveZero) * BigInt(1 * 10 ** decimal);
  let belowZeroBigInt = BigInt(0);
  const belowZero = list[1];

  if (belowZero !== undefined) {
    belowZeroBigInt = BigInt(belowZero.substring(0, decimal).padEnd(decimal, "0"));
  }

  return aboveZeroBigInt + belowZeroBigInt;
};

var _config = /*#__PURE__*/new WeakMap();

var _identity = /*#__PURE__*/new WeakMap();

var _principal = /*#__PURE__*/new WeakMap();

var _ic = /*#__PURE__*/new WeakMap();

class AstroX {
  // set config(config) {
  //   this.#config = config
  // }
  get identity() {
    var _classPrivateFieldGet2;

    return (_classPrivateFieldGet2 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet2 === void 0 ? void 0 : _classPrivateFieldGet2.identity;
  }

  get principal() {
    return _classPrivateFieldGet(this, _principal);
  }

  constructor() {
    let userConfig = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

    _defineProperty(this, "meta", {
      features: ["wallet"],
      icon: {
        light: astroXLogoLight,
        dark: astroXLogoDark
      },
      id: "astrox",
      name: "AstroX ME"
    });

    _classPrivateFieldInitSpec(this, _config, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldInitSpec(this, _identity, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldInitSpec(this, _principal, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldInitSpec(this, _ic, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldSet(this, _config, {
      whitelist: [],
      providerUrl: "https://63k2f-nyaaa-aaaah-aakla-cai.raw.ic0.app",
      ledgerCanisterId: "ryjl3-tyaaa-aaaaa-aaaba-cai",
      ledgerHost: "https://boundary.ic0.app/",
      host: window.location.origin,
      dev: true,
      ...userConfig
    });
  }

  set config(config) {
    _classPrivateFieldSet(this, _config, { ..._classPrivateFieldGet(this, _config),
      ...config
    });
  }

  get config() {
    return _classPrivateFieldGet(this, _config);
  }

  async init() {
    _classPrivateFieldSet(this, _ic, await IC.create({
      useFrame: !(window.innerWidth < 768),
      signerProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#signer"),
      walletProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#transaction"),
      identityProvider: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#authorize"),
      permissions: [PermissionsType.identity, PermissionsType.wallet],
      ledgerCanisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
      ledgerHost: _classPrivateFieldGet(this, _config).ledgerHost,
      onAuthenticated: icInstance => {
        var _ref;

        _classPrivateFieldSet(this, _ic, (_ref = window.ic.astrox) !== null && _ref !== void 0 ? _ref : icInstance);

        _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText());

        _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);
      },
      dev: _classPrivateFieldGet(this, _config).dev
    }));

    const isConnected = await this.isConnected(); // TODO: figure out

    if (isConnected) {
      _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);

      _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText());
    }

    return true;
  }

  async isConnected() {
    return _classPrivateFieldGet(this, _ic).isAuthenticated();
  }

  async createActor(canisterId, idlFactory) {
    var _classPrivateFieldGet3;

    // TODO: move from @astrox/connection here?
    // // Fetch root key for certificate validation during development
    return (_classPrivateFieldGet3 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet3 === void 0 ? void 0 : _classPrivateFieldGet3.createActor(idlFactory, canisterId);
  }

  async connect() {
    var _classPrivateFieldGet4;

    await ((_classPrivateFieldGet4 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet4 === void 0 ? void 0 : _classPrivateFieldGet4.connect({
      useFrame: !(window.innerWidth < 768),
      signerProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#signer"),
      walletProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#transaction"),
      identityProvider: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#authorize"),
      permissions: [PermissionsType.identity, PermissionsType.wallet],
      ledgerCanisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
      ledgerHost: _classPrivateFieldGet(this, _config).ledgerHost,
      onAuthenticated: icInstance => {
        var _window$ic$astrox;

        _classPrivateFieldSet(this, _ic, (_window$ic$astrox = window.ic.astrox) !== null && _window$ic$astrox !== void 0 ? _window$ic$astrox : icInstance);

        _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText());

        _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity); // this.#address = this.#ic.wallet

      }
    })); // TODO: Result type

    return true;
  }

  async disconnect() {
    var _classPrivateFieldGet5;

    await ((_classPrivateFieldGet5 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet5 === void 0 ? void 0 : _classPrivateFieldGet5.disconnect());
    return true;
  }

  address() {
    return {
      principal: _classPrivateFieldGet(this, _principal) // accountId: this.#ic.accountId,

    };
  }

  async requestTransfer(_ref2) {
    let {
      amount,
      to,
      from // TODO: fix return type

    } = _ref2;
    let result;

    try {
      var _classPrivateFieldGet6;

      result = await ((_classPrivateFieldGet6 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet6 === void 0 ? void 0 : _classPrivateFieldGet6.requestTransfer({
        amount: balanceFromString(String(amount)),
        to,
        from,
        // TODO: ?
        sendOpts: {}
      }));
    } catch (e) {
      return false;
    }

    switch (result.kind) {
      case "transaction-client-success":
        return {
          height: Number(result.payload.blockHeight)
        };

      default:
        return false;
    }
  } // TODO: gets called often


  async queryBalance() {
    var _Number, _classPrivateFieldGet7;

    const ICPBalance = (_Number = Number(await ((_classPrivateFieldGet7 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet7 === void 0 ? void 0 : _classPrivateFieldGet7.queryBalance()))) !== null && _Number !== void 0 ? _Number : 0;
    return [{
      amount: ICPBalance / 100000000,
      canisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
      decimals: 8,
      // TODO: fix
      image: "Dfinity.svg",
      name: "ICP",
      symbol: "ICP"
    }];
  }

  async signMessage(_ref3) {
    var _classPrivateFieldGet8;

    let {
      message
    } = _ref3;
    return (_classPrivateFieldGet8 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet8 === void 0 ? void 0 : _classPrivateFieldGet8.signMessage({
      signerProvider: _classPrivateFieldGet(this, _config).providerUrl,
      message
    });
  } // getManagementCanister: (...args) => this.#ic.getManagementCanister(...args),
  // batchTransactions: (...args) => this.#ic.batchTransactions(...args),


}

export { AstroX };
