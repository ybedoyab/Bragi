    ...
    const promises = uniquePaths.map((path, index) => {
        return (async () => {
            try {
                ...
                const cert = new __1.Certificate(response, agent);
                const verified = await cert.verify();
                if (!verified) {
                    throw new Error('There was a problem certifying the response data. Please verify your connection to the mainnet, or be sure to call fetchRootKey on your agent if you are developing locally');
                }
