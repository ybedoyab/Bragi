...
class AuthClient {
    ...
    static async create(options = {
        ...
        var _a;
        const storage = (_a = options.storage) !== null && _a !== void 0 ? _a : new icStorage_1.ICStorage('astrox-');
        let key = null;
        if (options.identity) {
            key = options.identity;
        }
        else {
            const maybeIdentityStorage = await storage.get(icStorage_1.KEY_ICSTORAGE_KEY);
        ...
        if (key) {
            try {
                const chainStorage = await storage.get(icStorage_1.KEY_ICSTORAGE_DELEGATION);
                wallet = await storage.get(icStorage_1.KEY_ICSTORAGE_WALLET);
                if (chainStorage) {
    ...
    async login(options) {
        ...
        if (!key) {
            ...
            this._key = key;
            await this._storage.set(icStorage_1.KEY_ICSTORAGE_KEY, JSON.stringify(key));
        }
            ...
            switch (message.kind) {
                ...
                case 'authorize-client-success':
                    ...
                    try {
                        ...
                        if (this._chain) {
                            await this._storage.set(icStorage_1.KEY_ICSTORAGE_DELEGATION, JSON.stringify(this._chain.toJSON()));
                        }
                        if (this._wallet !== undefined) {
                            await this._storage.set(icStorage_1.KEY_ICSTORAGE_WALLET, this._wallet);
                        }
                    }
                    catch (err) {
